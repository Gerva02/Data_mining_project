...
```{r}
(norm_glass_test<-apply(glass.test[,-ncol(glass)], MARGIN = 2, normalize))
#gervi così non ti piace????
norm_glass
```
...
plot(1:Kmax,knn.pred.k,type="h")
#abline(v=which.max(knn.pred.k),h=max(knn.pred.k)) 
#abline ha senso se cambiamo "type="
```

```{r}
lab<-as.factor(as.matrix(norm_glass[,ncol(glass)]))
knn.pred <- knn(train=as.matrix(norm_glass[,-ncol(glass)]), test=as.matrix(norm_glass_test), cl=lab, k = 3)
knn.pred
class(norm_glass[,-ncol(glass)])
class(factor(norm_glass[,ncol(glass)]))
class(norm_glass_test)
