...
a3 <- as.vector(c( sos$RI[1]$lambda,sos$Na[1]$lambda , sos$Mg[1]$lambda, sos$Al[1]$lambda, sos$Si[1]$lambda,sos$Ca[1]$lambda))
colnames(p)
col
...
transformed.glass <- tibble(cbind(as.data.frame(transformedY)), Type = labels_train)
...
set.seed(123)
knn.pred <- knn(train=train_norm, test=test_norm, cl=labels_train, k = best.res %>% filter(metodo=="knn") %>% pull(hyperparameter)) 
