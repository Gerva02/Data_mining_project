...
Si notano diversi split che sembrano poco utili per la classificazione alla fine dell'albero. Questi risultano infatti nella stessa classe e potrebbero essere in futuro potati tramite tencniche di pruning.
...
Una seconda problematica è la elevata multicollinerità riscontrata nella fase di preprocessing tra le variabili *Ca* e *RI*. A livello teorico una di queste due variabili andrebbe quindi omessa, ma in seguito ad analisi approfondite è risultato più performante un modello basato sulla presenza di entrambe le variabili.
...
Anche in questo caso si riporta lo stesso grafico di prima per mostrare tutti i k possibili e la loro relativa accuratezza.purtroppo la scelta del parametro k ricade su un valore molto elevato e determina una inevitabile influenza sulla previsione da parte delle classi più numerose, causando novetoli problemi in fase di valutazione del modello e utilizzo poi in un futuro per scopi reale. Per questo motivo in seguito si opterà per un valore massimo di k inferiore.
...
È interessante sottolineare come tutti i metodi preferiscano la scelta di un modello in genrale più complesso che rischia di incombere in problemi di overfitting, riscontrando sì un bias inferiore ma trovando molte difficoltà nell'effetturare previsione su dati sconosciuti al modello.
...
Il procedimento è analogo a quanto visto nel capitolo precedente: tramite cross validation sul training, sempre con lo stesso split, vengono trovati i 3 diversi iperparametri realtivi ai 3 metodi, per poi calcolarne l'accuracy.
