...
Nel lavoro verranno utilizzate e confrontate diverse tecniche di classificazione andando a cercare la più accurata.
...
Di seguito è riportata comunque una verifica pratica nel nostro training set andando a cercare tutte le osservazioni con valori positivi. L'output fornisce un tibble con tutte le 171 osservazioni del training, ovvero l'80% del dataset completo, andandoci a confermare che non abbiamo valori negativi.
...
Si notano diversi split che sembrano poco utili per la classificazione alla fine dell'albero. Questi splittano infatti nella stessa classe e potrebbero essere in futuro potati tramite tencniche di pruning.
...
Un approccio alternativo al classico algoritmo del KNN è quello di utilizzare le distanze di Mahalanobis al posto delle distanze euclidee come metodo di misura della distanza. In questo caso non è neanche necessaria la normalizzazione, dato che la distanza di Mahalanobis tiene conto delle correlazioni e include una trasformazione che già pone le variabili sulla stessa scala.
...
È stata effettuata anche una regressione logistica multiclasse, ma è stata esclusa a causa di scarsi risultati (circa 0,40 di accuracy).

In questa fase verrà tuttavia utilizzato un metodo diverso per trovare il modello migliore, ovvero la tecnica della **cross-validation**. Tramite questo metodo più robusto si riescono a trovare iperparametri migliori sia per gli alberi decisionali che per il KNN.
...
Ovviamente verranno mantenuti sempre gli stessi gruppi per avere un confronto equo tra metodi.
...
Il metodo migliore sembra per ora essere il KNN con distanze euclidee con un'accuracy di 0.64, ma anche gli altri metodi danno buoni risultati intorno al 60%.
...
È utile far notare l'interpretazione opposta del valore dell'iperparametro nel KNN e negli alberi decisionali. Nel KNN infatti più il parametro è grande e più il modello è semplice, avendo quindi un maggiore bias e una minore varianza. Al contrario, negli alberi decisionali si ha un modello più complesso con l'aumentare del parametro, con minore bias e maggiore varianza.
...
Il procedimento è analogo a quanto visto il capitolo precedente: tramite cross validation sul training, sempre con lo stesso split, vengono trovati i 3 diversi iperparametri realtivi ai 3 metodi, per poi calcolarne l'accuracy.
Anche in questo caso vengono riportati una tabella riassuntiva ed il grafico con tutti possibili k e le accuracy relative.

I risultati ottenuti sono deludenti per il KNN con Mahalanobis (accuracy pari a 0.38), mentre sono molto buoni per quanto riguarda il KNN normale (accuracy pari a 0.72).
...
* Il metodo migliore, come visto nei paragrafi precedenti, è quindi il KNN con distanze euclidee, applicato sulle 6 variabili trasformate con Box-Cox e normalizzate: il k ottimale è k=3.
...
Viene riportata anche la Confusion Matrix. Si nota come il metodo faccia fatica in alcune classi minoritarie, come la terza, probabilmente a causa di poche osservazioni e data la semplicità dell'algoritmo.
Avendo a disposizione un maggior numero di dati si potrebbero fare analisi più approfondite su ciascuna delle classi e delle vairabili, ovvero sulle varie tipologie di vetro e la loro composizione.
...
La regressione logistica multiclasse è stata invece esclusa a priori perchè dava risultati ancora peggiori. 
