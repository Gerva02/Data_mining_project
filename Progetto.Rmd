...
for (j in 1:cv){
  ...
  pruned.tree.k <-prune.tree(tree.glass.cv ,method="misclass")$k
  tree.size.possible <-length(prune.tree(tree.glass.cv ,method="misclass")$k)-2
  #metodo alberi
  for (i in 1:tree.size.possible){
    ...
    acc <- mean(tree.pred == labels_train[split ==j])
    #l'iper parametro da salvare è anche k 
  }
}
...
knn.pred <- knn(train=train_norm, test=test_norm, cl=labels_train, k = 4)
mean(glass.test$Type == knn.pred)
```

```{r}
train_norm<-glass.train[,-k.train]
test_norm<-glass.test[,-k.train]
k_best  <- 2#una volta trovato il k migliore 

tree.best.glass <- tree(Type ~. , glass.train)
pruned.tree.k <-prune.tree(tree.glass.cv ,method="misclass",k=k_best)

tree.pred <- predict(pruned.tree.k , glass.test[-10] , type = "class")
mean(tree.pred == glass.test$Type) 
