...
transformed.glass <- tibble(cbind(as.data.frame(transformedY)), Type = glass.train$Type
)
...
predict_label <- function(v, train, mat, k, train_lab) {
  ...
  check <- cbind(dists, train_lab)
  colnames(check) <- c("dist", "lab")
  check <- as.tibble(check)
  check_ord <- top_n(check, wt = dist, n = k)
  return(mode(check_ord$lab))
}
...
knn_mahalanobis <- function(train, valid, train_lab, valid_lab, k) {
  ...
  pred <- sapply(as.data.frame(t(valid)), function(x) predict_label(x, train, x_matrix_inv, k, train_lab))
