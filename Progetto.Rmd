...
mahalanobis_dist <- function(x, center, cov_inv) {
  diff_vec <- t(t(x) - center)
  mahalanobis_dists <- sqrt(rowSums((diff_vec %*% cov_inv) * diff_vec))
  return(mahalanobis_dists)
}

x_mean<-colMeans(glass.train[,-k])
x_matrice_var_cov<-cor(glass.train[,-k])

X_train_mahala<-mahalanobis_dist(glass.train[,-k],x_mean,solve(x_matrice_var_cov))
X_train_mahala #non mi torna????
```
