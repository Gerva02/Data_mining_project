...
for (j in 1:cv){
  split<-sample(c(T,F), size=n.train, replace = T, prob = c(p,1-p)) #questo è sbagliato
  out_norm_cv<-normalize_2(glass.train[split,-k.train],glass.train[!split,-k.train])
  train_cv<-out_norm_cv$train_norm
  test_cv<-out_norm_cv$test_norm
  #metodo knn
  for (i in 1:knn.kmax){
    set.seed(42)
    knn.pred <- knn(train=train_cv, test=test_cv, cl=labels_train[split], k = i)
    #	knnMCN(TrnX, OrigTrnG, TstX = NULL, K = 1, ShowObs = F) prova a vedere se questa risolve i problemi
    ...
    mat[i,j+2]<-mean(knn.pred==labels_train[!split])
  }
...
knn.pred <- knn(train=glass.train, test=glass.test, cl=labels_train, k = 4)
