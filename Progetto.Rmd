...
Si è deciso di non trasformare le 3 variabili *Ba*, *Fe* e *K* analizzate precedentemente. Queste presentavano valori non solo asimmetrici, ma molto spostati verso lo zero, rendendo quindi ininfluenti e inutili le trasformazioni. Nella fase successiva si procederà quindi ad escluderle totalmente in alcune analisi.
...
Viene utilizzato il Test di Shapiro-Wilk chge verifica tramite con l'ipotesi nulla la normalità del campione.
Sono di seguito riportate due tabelle, con la suddivisione in gruppi di cui sopra:

* Nella prima ci sono i p-value del test per ognuna delle 9 variabili iniziali non trasformate: è imputato un valore NA per i 3 elementi chimici con tutti valori pari a zero nella classe 6.
* 
...
```
#dealing with multicollinearity (vedremo se metterla...)
...
predict_label <- function(v, train, mat, k, train_lab) {
  ...
  print(check)
  check_ord <-  check  %>% slice_min(dist, n = 5)
  print(check_ord)
...
knn_mahalanobis(glass.train[,1:9], glass.test[,1:9], glass.train$Type, glass.test$Type, k=5) 
#NON VA
```
...
knn.kmax<-10
tree.size.max<- length(pruned.tree.k)-2
mat.2<-as.data.frame(matrix(NA,nrow=(knn.kmax+tree.size.max),ncol=cv+2))
