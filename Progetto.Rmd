...
```
Vengono di seguito riportati i boxplot di ciascuna delle variabili, senza aver effettuato nessun tipo di trasformazione. Si nota in alcune una distribuzione abbastanza sbilanciata, con la presenza di un'asimmetria. Altre variabili sembrano invece abbastanza simmetriche e senza la presenza di alcun outlier.
...
È utile osservare anche il Vilion plot delle variabili condizionato alle classi.
Tramite questo grafico si riescono infatti a notare 3 cose:
...
vediamo inoltre che per type = 6 BA FE K non variano assolutamente ( magari fare un altro grafico) (sono tutti 0, potremo toglierla però ritengo che semplicemente possono essere elementi chimici non presenti in quel vetro)
...
```{r test knn nomr}
out_norm <- normalize_2(glass.train[,-10],glass.test[,-10]) #k è lo stesso in ogni caso dai
train_norm<-out_norm$train_norm
test_norm<-out_norm$test_norm



knn.pred <- knn(train=train_norm, test=test_norm, cl=labels_train, k = 2) #non funziona se ci escono 2 o più soluzioni
...
pruned.best.tree <-prune.tree(tree.best.glass  ,method="misclass", k = 3) 
...
```{r test trees variabili trasformate}
#senza normalizzare 


tree.best.glass <- tree(Type ~. , transformed.glass.train)
pruned.best.tree <-prune.tree(tree.best.glass  ,method="misclass", k = 1) 
